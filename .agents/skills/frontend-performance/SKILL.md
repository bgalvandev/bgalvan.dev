...
description: Keep the portfolio fast and fluid — rely on the React Compiler instead of manual memoization, stream with Suspense, virtualize large lists, and keep the client bundle small. Use when a UI feels slow, renders large/long data, or you are tempted to add useMemo/useCallback/memo.
---

# Frontend Rendering Performance

The React Compiler is **enabled** (`reactCompiler: true` in `next.config.js`,
backed by `babel-plugin-react-compiler`). It auto-memoizes components and hooks at
build time. Work _with_ it.
...
- The `eslint-plugin-react-hooks` v7 rules (`recommended-latest`, wired in
  `eslint.config.mjs`) enforce the Rules of React the compiler depends on
...
- Fetch in **parallel**: place independent server-side reads in sibling Server
  Components (or `Promise.all`), never awaited one-after-another in a chain.

## Cut waterfalls and redundant work

- The biggest server-render cost is usually **request waterfalls**: awaiting one read
...
- Bound the content a page renders so the client never receives an unbounded payload.
  Slice/paginate large collections (e.g. a long list of projects or posts) at the
  data source in `src/content/**` or `src/lib/**` before handing them to the UI.
...
- `pnpm run build` succeeds with the compiler enabled.
- `pnpm run lint` passes (Rules-of-React lint active).
